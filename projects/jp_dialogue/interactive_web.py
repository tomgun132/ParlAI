...
class MyHandler(BaseHTTPRequestHandler):
    ...
    def _interactive_running(self, opt, reply_text):
        if len(self.history) > SHARED['opt'].get('hist_size'):
            del self.history[0]
        SHARED['conv_history']['input'].append(reply_text)
        reply_text = SHARED['tokenizer'].parse(reply_text)
        print(reply_text)
        ...
        output = {"text": resp}
        self.history.append(resp_cands[0])
        SHARED['conv_history']['response'].append(resp)
        SHARED['conv_history']['candidates'].append(resp_cands)
        SHARED['conv_history']['candidates_scores'].append(model_res['scores'].tolist())
        print(output, self.history)
        return output
...
if __name__ == '__main__':
    ...
    httpd = ThreadedHTTPServer((HOST_NAME, opt['port']), MyHandler)
    print('http://{}:{}/chatbot'.format(HOST_NAME, opt['port']))
    ...
    try:
        ...
        with open(SHARED['opt']['datapath'] + "/models/rachel/logs/logs_%s.csv" % time, 'w', encoding='utf-8') as f:
            ...
            # json.dump(SHARED['conv_history'], f)
        httpd.server_close()
