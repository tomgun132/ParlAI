...
def learn_nidf(opt):
    ...
    print('Counting words in Rachel train set...')
    opt['datatype'] = 'train:ordered'
    opt['fromfile_datapath'] = os.path.join(
        opt['datapath'], RACHEL_DIR, 'train.txt'
    )
    ...
    num_sents = ns1 + ns2 + ns3

    # Write word2count and num_sents to file
    word2count_fp = os.path.join(opt['datapath'], RACHEL_DIR, 'word2count.pkl')
