...
class SentenceEmbedder(object):
    ...
    for words w with fasttext embeddings word_emb(w).
    ...
    def __init__(
        self, word2prob, arora_a, fasttext_name, fasttext_dim, first_sv, fasttext_cache
    ):
        """
          Inputs:
            ...
            fasttext_name: the version of fasttext to use, e.g. '840B'
            fasttext_dim: the dimension of the fasttext embeddings to use, e.g. 300
            first_sv: np array shape (fasttext_dim). The first singular value,
              used to compute Arora sentence embeddings. Can be None.
            fasttext_cache: The path to where the fasttext vectors are stored.
        """
        self.word2prob = word2prob
        self.arora_a = arora_a
        self.fasttext_name = fasttext_name
        self.fasttext_dim = fasttext_dim
        self.fasttext_cache = fasttext_cache
        ...
        self.tt_embs = None  # will be torchtext.vocab.fasttext object
        self.emb_matrix = None  # will be np array shape (vocab_size, fasttext_dim)
    ...
    def get_fasttext_embs(self):
        """
        Loads torchtext fasttext embs from file and stores in self.tt_embs.
        """
        print('Loading torchtext fasttext embs (for Arora sentence embs)...')
        self.tt_embs = vocab.Vectors(
            name=self.fasttext_name, url=URL, cache=self.fasttext_cache
        )
        print('Finished loading fasttext fasttext embs')

    def get_emb_matrix(self, dictionary):
        """
        Construct an embedding matrix containing pretrained fasttext vectors for all words
        ...
        """
        print(
            'Constructing fasttext emb matrix for response-relatedness weighted '
            'decoding...'
        )
        self.emb_matrix = []
        ...
        for idx in range(len(dictionary)):
            ...
            if word in self.tt_embs.stoi:
                ...
                word_emb = torch.zeros(self.fasttext_dim)
                oov_indices.append(idx)
            self.emb_matrix.append(word_emb)
        self.emb_matrix = np.stack(self.emb_matrix)  # (vocab_size, fasttext_dim)
        print(
            'Done constructing fasttext emb matrix; found %i OOVs of %i words'
    ...
    def get_word_sims(self, sent, sent_emb, dictionary):
        ...
        Inputs:
          ...
          sent_emb: torch Tensor shape (fasttext_dim).
          dictionary: ParlAI dictionary

        Returns:
          sims: torch Tensor shape (vocab_size), containing the cosine sims.
        """
        # If we haven't initialized the fasttext emb matrix yet, do so
    ...
    def embed_sent(self, sent, rem_first_sv=True):
        ...
        Returns:
          sent_emb: tensor length fasttext_dim, or None.
              If sent_emb is None, that's because all of the words were OOV for fasttext.
        """
        # If we haven't loaded the torchtext fasttext embeddings, do so
        if self.tt_embs is None:
            self.get_fasttext_embs()

        # Lookup fasttext embeddings for words
        tokens = [t for t in sent if t in self.tt_embs.stoi]  # in-vocab tokens
        # fasttext_oov_tokens = [t for t in sent if t not in self.tt_embs.stoi]
        # if len(fasttext_oov_tokens)>0:
        #     print("WARNING: tokens OOV for fasttext: ", fasttext_oov_tokens)
        if len(tokens) == 0:
            print(
                'WARNING: tried to embed utterance %s but all tokens are OOV for '
                'fasttext. Returning embedding=None' % sent
            )
            return None
        word_embs = [
            self.tt_embs.vectors[self.tt_embs.stoi[t]] for t in tokens
        ]  # list of torch Tensors shape (fasttext_dim)
        ...
        sent_emb = sum(
            ...
            word_embs
        )  # torch Tensor shape (fasttext_dim)
...
def remove_first_sv(emb, first_sv):
    ...
    Inputs:
      emb: torch Tensor shape (fasttext_dim)
      first_sv: torch Tensor shape (fasttext_dim)

    Returns:
      new emb: torch Tensor shape (fasttext_dim)
    """
    # Calculate dot prod of emb and first_sv using torch.mm:
    # (1, fasttext_dim) x (fasttext_dim, 1) -> (1,1) -> float
...
def learn_arora(opt):
    ...
    """
    arora_file = os.path.join(opt['datapath'], 'rachel', 'arora.pkl')
    ...
    total_count = total_count_train

    # Merge all_utts
    all_utts = all_utts_train
    ...
    arora_a = 0.0001
    fasttext_name = 'cc.ja.300.vec.gz'
    fasttext_dim = 300
    fasttext_cache = model_path(opt['datapath'], 'fasttext_cc_jp_vectors')
    ...
    utt_embs = np.stack(utt_embs, axis=0)  # shape (num_embs, fasttext_dim)
    U, s, V = np.linalg.svd(utt_embs, full_matrices=False)
    first_sv = V[0, :]  # first row of V. shape (fasttext_dim)
    ...
    utt_embs_adj = [
        ...
        for emb in utt_embs
    ]  # list of np arrays shape (fasttext_dim)
    ...
    # info about fasttext vectors used, and full dict of utt->emb to file
        ...
        pickle.dump(
            {
                ...
                'first_sv': first_sv,  # np array shape (fasttext_dim)
                'arora_a': arora_a,  # float, 0.0001
                'fasttext_name': fasttext_name,  # string, 'cc.ja'
                'fasttext_dim': fasttext_dim,  # int, 300
                'utt2emb': utt2emb,  # dict: string to np array shape (fasttext_dim)
            },
            f,
        )
...
def load_arora(opt):
    ...
    """
    arora_fp = os.path.join(opt['datapath'], RACHEL_DIR, 'arora.pkl')
